...
//-------------		Kopie von Linux-Version  			------------
//------------------------------------------------------------------
static void StoppeAlleSounds ()
{
  int i;
  SDL_PauseAudio(1); // Stoppe das Audioausgabe-System!
  SDL_LockAudio();   // Sperre den Zugriff!  
  // Lösche nun alle Audiopuffer!
  for (i=0; i < MAX_PLAYING_SOUNDS;i++) {
	if (playing[i].active ==1) {
	  free(s[i].samples);    // Samplespeicher frei geben
	  playing[i].active = 0; // und diesen "Samplekanal" auf ungenutzt setzen
	}
  }
  SDL_UnlockAudio();
  SDL_PauseAudio(0); // Starte das Audioausgabe-System wieder!
}    
//-----------------------------------------------------------------
...
func f (anfrage string) string {
	...
	switch a[0] {
		...
		SpieleSound(name)
		return "OK"
		// Extra um StoppeAlleSounds() auch in Windows zu nutzen
		case "STAS":
		if len (a) != 1 {return "ERROR1" }
		//name:= a[1]
		//for i:=2;i<len(a);i++ {name = name+":"+a[i]}
		StoppeAlleSounds() //SpieleSound(name)
		return "OK"
		///////////////////////////////77
